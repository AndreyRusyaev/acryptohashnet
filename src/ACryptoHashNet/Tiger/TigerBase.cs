...
namespace acryptohashnet
{
    ...
    /// </summary>
    public abstract class TigerBase : BlockHashAlgorithm
    {
        ...
        protected override void ProcessBlock(byte[] array, int offset)
        {
            ...
            // key schedule
            buffer[0] -= buffer[7] ^ 0xa5a5a5a5a5a5a5a5;
            buffer[1] ^= buffer[0];
            buffer[2] += buffer[1];
            buffer[3] -= buffer[2] ^ ((~buffer[1]) << 19);
            buffer[4] ^= buffer[3];
            buffer[5] += buffer[4];
            buffer[6] -= buffer[5] ^ ((~buffer[4]) >> 23);
            buffer[7] ^= buffer[6];

            buffer[0] += buffer[7];
            buffer[1] -= buffer[0] ^ ((~buffer[7]) << 19);
            buffer[2] ^= buffer[1];
            buffer[3] += buffer[2];
            buffer[4] -= buffer[3] ^ ((~buffer[2]) >> 23);
            buffer[5] ^= buffer[4];
            buffer[6] += buffer[5];
            buffer[7] -= buffer[6] ^ 0x0123456789abcdef;
